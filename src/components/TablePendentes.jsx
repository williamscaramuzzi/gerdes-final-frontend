...
import { COLUMNS } from './columnspendentes';
...
export default function Pendentes(props) {
    ...
    async function buscar() {
        let retorno = await fetch("http://localhost:3000/ocorrenciaspendentes");
        ...
        <table id="pendentes"  {...getTableProps()}>
            ...
            <tbody {...getTableBodyProps()}>
                {rows.map((row, i) => {
                    ...
                    return (
                        <tr {...row.getRowProps()}>
                            <td><button id={row.cells[0].value} onClick={(e)=>props.enviarOcoEdicao(e.target.id)}>Editar</button></td>
                            {row.cells.map(cell => {
