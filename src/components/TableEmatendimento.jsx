...
import { COLUMNS } from './columnsEmatendimento';

import './TableEmatendimento.css';

export default function Ematendimento(props) {
    const [conteudo, setConteudo] = useState([]);
    async function buscar() {
        let retorno = await fetch("http://localhost:3000/ocorrenciasematendimento");
    ...
    return (
        <table id="emAtendimento"  {...getTableProps()}>
            <thead>
                {
            ...
            <tbody {...getTableBodyProps()}>
                {rows.map((row, i) => {
                    ...
                    return (
                        <tr {...row.getRowProps()}>
                            <td><button id={row.cells[0].value} onClick={(e)=>props.enviarOcoConclusao(e.target.id)}>Editar</button></td>
                            {
                                row.cells.map(cell => {
